    ...
    switch(choice)
    {
      ...
      case '3':
        ...
        else
        {
          ...
          show_bookshop_list(*first);
          std::cin>>number; //wprowadzic kontrole
...
void bookshop_menu2(Bookshop *first)
{
  ...
  while(ii == 1){
    ...
    std::cout<<"5. Zarzadzaj zamowieniami"<<std::endl; //wyswietla zamowienie - mFwybiozliwosc odrzucenia lub zaakceptowania
    ...
    switch(choice)
    {
      ...
      case 3:
        ...
        else
        {
          ...
          current->show_employee_list();  //zastanowic sie nad mechanizmem sprawdzania bledow
          std::cin>>number;
...
void customer_menu1(Bookshop *first)
{
  ...
  while(ii == 1)
  {
    std::cout<<endl<<"KLIENT - ksiegarnia "<<current->get_name()<<" w miescie "<<current->get_city()<<std::endl;
    ...
    switch (choice)
    {
      ...
      case 4:
        if(current->get_firstC() == NULL)
          ...
          std::cout<<"Klient podchodzi do drzwi ksiegarni. \"ZAMKNIETE DO ODWOLANIA\" - glosi napis na drzwiach"<<endl;
...
void customer_menu2(Customer *currentC, Bookshop *currentB)
{
  ...
  while(ii == 1){
    ...
    std::cout<<" wchodzi do ksiegarni "<<currentB->get_name()<<" w miescie "<<currentB->get_city();
    std::cout<<". Podchodzi do lady"<<std::endl<<"-W czym moge pomoc? - pyta sprzedawca"<<std::endl;
    std::cout<<"-Chwilowo sie tylko rozgladam - odpowiada klient"<<endl;
    std::cout<<"1. Obejrzyj ksiazki"<<std::endl;  //wyswietla liste ksiazek
    ...
    switch(choice)
    {
      ...
      case 2:
      {
       ...
       // Book *currentBo;
        ...
        if(currentB->get_firstB() != NULL && nB != 0)  //uwaga na przypadek, gdy ksiazek nie ma wcale
        {
          ...
          if(currentC->get_money() >= currentBo->get_price())  //rok i strony - rozwiazac problem przy losowaniu
          {
